...
import { sendEmail, EMAIL_CONFIG } from '@/lib/ses'
import { makeBarChart, makeTrendChart } from '@/lib/charts'

// Helper to extract a numeric value from various CSV fields
...
function matchTickerToPrice(ipo: any, recentPrices: any[], tickerPrices: any[]): { proposed_price_low: number; proposed_price_high: number } {
  ...
  // Try recent CSV price tables for missing values
...
export async function POST(request: NextRequest) {
	try {
		...
		let requestedRecipients = body.emails ?? body.recipients ?? (body.email ? [body.email] : []);
		...
		console.log(`Preview newsletter type: ${newsletterDay === 'monday' ? 'Recent IPOs' : 'Upcoming IPOs'}`);

		let scoredIPOs: any[] = [];
		
		// Fetch IPO data from PythonAnywhere CSVs
		let ipoList: any[] = [];
		let ipoCalendarRows: any[] = [];
		...
		if (ipoList.length === 0) {
			return NextResponse.json({ 
				...
				message: 'Unable to fetch IPO data. Please check PythonAnywhere CSV files.' 
			}, { status: 503 })
		}
		...
		try {
			...
			if (calendarResponse.ok) {
				...
				if (calendarData.success && calendarData.data) {
					ipoCalendarRows = calendarData.data;
				}
			}
		} catch (error) {
		...
		const calendarSource = ipoCalendarRows.length > 0 ? ipoCalendarRows : ipoList;
		...
		const topFive = scoredIPOs.slice(0, 5);

		// Check if we have any IPOs to show
		if (topFive.length === 0) {
			return NextResponse.json({ error: 'No IPO hype scores were generated after processing up to 10 IPOs.' }, { status: 503 });
		}

		// Charts
		const barChartUrl = makeBarChart(topFive)
		...
		const trendChartUrl = makeTrendChart(topFive[0].name, trendData)
		const metricsChartUrl = makeBarChart(topFive) // Using bar chart for metrics

		// Generate comprehensive market overview
		const avgHypeScore = topFive.reduce((sum, ipo) => sum + ipo.hypeScore, 0) / topFive.length
		const highPerformers = topFive.filter(ipo => ipo.hypeScore > 75).length
		const avgRevenueGrowth = topFive.reduce((sum, ipo) => sum + ipo.revenueGrowthYoY, 0) / topFive.length
		
		const newsletterSummary = `This week's IPO market analysis reveals ${highPerformers} high-performing companies (hype score >75) out of ${topFive.length} analyzed, with an average hype score of ${avgHypeScore.toFixed(1)}/100. The market shows strong momentum with average revenue growth of ${(avgRevenueGrowth * 100).toFixed(1)}% YoY across analyzed companies. ${topFive.filter(ipo => ipo.recommendation === 'Buy').length} companies received 'Buy' recommendations based on historical analysis, while ${topFive.filter(ipo => ipo.riskLevel === 'High').length} are classified as high-risk investments. The analysis incorporates comprehensive historical data from similar IPOs, search trend patterns, and sentiment analysis to provide statistically-backed insights for each company's investment potential.`

		// Unsubscribe URL (no token for preview)
		...
		// Send the preview email using AWS SES
		await sendEmail({
			to: recipients,
			subject: `[Preview] ${EMAIL_CONFIG.subject}`,
			html,
		})
		...
		return NextResponse.json({ 
			...
			iposProcessed: topFive.length
		})
	} catch (error: any) {
		console.error('Preview send failed:', error)
		return NextResponse.json({ error: 'Failed to send preview', details: error?.message || String(error) }, { status: 500 })
	}
}